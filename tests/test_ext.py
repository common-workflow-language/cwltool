...
class TestListing(unittest.TestCase):
    def test_missing_enable_ext(self):
        ...
        self.assertEquals(main([get_test_data('wf/listing_deep.cwl'), get_test_data('listing-job.yml')]), 1)

    def test_listing_deep(self):
        # Should succeed.
        self.assertEquals(main(["--enable-ext", get_test_data('wf/listing_deep.cwl'), get_test_data('listing-job.yml')]), 0)
    ...
    def test_listing_shallow(self):
        ...
        self.assertEquals(main(["--enable-ext", get_test_data('wf/listing_shallow.cwl'), get_test_data('listing-job.yml')]), 1)

    def test_listing_none(self):
        # This fails on purpose, because it tries to access listing but it shouldn't be there.
        self.assertEquals(main(["--enable-ext", get_test_data('wf/listing_none.cwl'), get_test_data('listing-job.yml')]), 1)

    def test_listing_v1_0(self):
         # Default behavior in 1.0 is deep expansion.
         self.assertEquals(main([get_test_data('wf/listing_v1_0.cwl'), get_test_data('listing-job.yml')]), 0)

    # def test_listing_v1_1(self):
    #     # Default behavior in 1.1 will be no expansion
    #     self.assertEquals(main([get_test_data('wf/listing_v1_1.cwl'), get_test_data('listing-job.yml')]), 1)
...
class TestInplaceUpdate(unittest.TestCase):
    ...
    def test_updateval(self):
        try:
            ...
            self.assertEquals(main(["--outdir", out, get_test_data('wf/updateval.cwl'), "-r", os.path.join(tmp, "value")]), 0)
    ...
    def test_updateval_inplace(self):
        try:
            ...
            with open(os.path.join(tmp, "value"), "w") as f:
                f.write("1")
            out = tempfile.mkdtemp()
            self.assertEquals(main(["--enable-ext", "--leave-outputs", "--outdir", out, get_test_data('wf/updateval_inplace.cwl'), "-r", os.path.join(tmp, "value")]), 0)
    ...
    def test_write_write_conflict(self):
        try:
            ...
            self.assertEquals(main(["--enable-ext", get_test_data('wf/mut.cwl'), "-a", os.path.join(tmp, "value")]), 1)
            with open(os.path.join(tmp, "value"), "r") as f:
                self.assertEquals("2", f.read())
        finally:
            shutil.rmtree(tmp)
    ...
    def test_sequencing(self):
        try:
            ...
            self.assertEquals(main(["--enable-ext", get_test_data('wf/mut2.cwl'), "-a", os.path.join(tmp, "value")]), 0)
            with open(os.path.join(tmp, "value"), "r") as f:
    ...
    #         self.assertEquals(main(["--enable-ext", get_test_data('wf/mut3.cwl'), "-a", os.path.join(tmp, "value")]), 0)
    #     finally:
    #         shutil.rmtree(tmp)
    ...
    def test_updatedir(self):
        try:
            ...
            self.assertEquals(main(["--outdir", out, get_test_data('wf/updatedir.cwl'), "-r", tmp]), 0)
    ...
    def test_updatedir_inplace(self):
        try:
            ...
            self.assertEquals(main(["--enable-ext", "--leave-outputs", "--outdir", out, get_test_data('wf/updatedir_inplace.cwl'), "-r", tmp]), 0)
