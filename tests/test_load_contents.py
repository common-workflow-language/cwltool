...
@pytest.mark.skipif(
    not cwltool.builder.__file__.endswith(".py"),
    reason="Does not work if in a mypyc-compiled build.",
)
def test_load_contents_file_array_for_step_input(tmp_path: Path) -> None:
    ...
    with patch(
        "cwltool.workflow_job.content_limit_respected_read",
        wraps=cwltool.builder.content_limit_respected_read,
    ) as mock_read:
        assert main(params) == 0
