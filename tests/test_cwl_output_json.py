...
def test_cwl_output_json_missing_field_v1_0() -> None:
    ...
    err_code, stdout, _ = get_main_output(["--debug", get_data("tests/test-cwl-out-v1.0.cwl")])
    assert err_code == 0
...
def test_cwl_output_json_missing_field_v1_1() -> None:
    ...
    err_code, stdout, _ = get_main_output(["--debug", get_data("tests/test-cwl-out-v1.1.cwl")])
    assert err_code == 0
    assert "foo" in json.loads(stdout)
...
def test_cwl_output_json_missing_field_v1_2() -> None:
    ...
    err_code, stdout, _ = get_main_output(["--debug", get_data("tests/test-cwl-out-v1.2.cwl")])
    assert err_code == 0
    assert "foo" in json.loads(stdout)
