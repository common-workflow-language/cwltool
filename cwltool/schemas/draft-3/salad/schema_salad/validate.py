    ...
    if schema_type == 'null':
        if datum is None:
            ...
            raise ValidationException(
                "the value `%s` is not null" % vpformat(datum))
    elif schema_type == 'boolean':
        if isinstance(datum, bool):
            return True
        else:
            raise ValidationException(
                "the value `%s` is not boolean" % vpformat(datum))
    elif schema_type == 'string':
        if isinstance(datum, basestring):
            return True
        else:
            raise ValidationException(
                "the value `%s` is not string" % vpformat(datum))
    elif schema_type == 'bytes':
        if isinstance(datum, str):
            return True
        else:
            raise ValidationException(
                "the value `%s` is not bytes" % vpformat(datum))
        ...
        if ((isinstance(datum, int) or isinstance(datum, long))
            ...
            raise ValidationException(
                "the value `%s` is not long" % vpformat(datum))
        ...
        if isinstance(datum, str) and len(datum) == expected_schema.size:
            ...
            raise ValidationException(
                "the value `%s` is not fixed" % vpformat(datum))
        ...
        if isinstance(datum, list):
            for i, d in enumerate(datum):
                try:
                    validate_ex(
                        expected_schema.items, d, identifiers, strict=strict,
        ...
        if (isinstance(datum, dict) and
            ...
            raise ValidationException("`%s` is not a valid map value, expected\n %s" %
                                      (vpformat(datum), vpformat(expected_schema.values)))
            ...
            for d in datum:
                ...
                if not found:
                    if d not in identifiers and d not in foreign_properties and d[0] not in ("@", "$"):
                        ...
                        if split.scheme:
                            errors.append(
                                "could not validate extension field `%s` because it is not recognized and strict is True.  Did you include a $schemas section?" % (d))
                        else:
                            errors.append("could not validate field `%s` because it is not recognized and strict is True, valid fields are: %s" %
