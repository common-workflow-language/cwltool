...
def get_image(dockerRequirement, pull_image, dry_run=False):
    ...
    if ("dockerImageId" not in dockerRequirement and
            "dockerPull" in dockerRequirement):
    ...
    for ln in subprocess.check_output(
        ...
        try:
            ...
            if ((sp[0] == m.group(1) and sp[1] == m.group(2)) or
    ...
    if not found and pull_image:
        if "dockerPull" in dockerRequirement:
            ...
            cmd = ["docker", "build", "--tag=%s" %
                   dockerRequirement["dockerImageId"], dockerfile_dir]
            _logger.info(str(cmd))
            if not dry_run:
                subprocess.check_call(cmd, stdout=sys.stderr)
                found = True
            ...
            if not dry_run:
                if os.path.exists(dockerRequirement["dockerLoad"]):
                    ...
                    _logger.info("Sending GET request to %s",
                                 dockerRequirement["dockerLoad"])
                    req = requests.get(
                        dockerRequirement["dockerLoad"], stream=True)
                    n = 0
            ...
            cmd = ["docker", "import", dockerRequirement[
                "dockerImport"], dockerRequirement["dockerImageId"]]
            _logger.info(str(cmd))
            if not dry_run:
                subprocess.check_call(cmd, stdout=sys.stderr)
                found = True
